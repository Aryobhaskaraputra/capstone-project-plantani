...
const getUserById = async (userId) => {
  const SQLQuery = `SELECT users.name, users.username, users.email, users.password, users.phoneNumber, locations.locationName
                    FROM users
...
const addMyPlants = async (userId, plantId) => {
  ...
  const hashMyPlants = await generateHash(
    userId.toString() + plantId.toString()
  );
  const SQLQuery =
    "INSERT INTO myplants (myplant_id, user_id, plant_id) VALUES (?, ?, ?)";
